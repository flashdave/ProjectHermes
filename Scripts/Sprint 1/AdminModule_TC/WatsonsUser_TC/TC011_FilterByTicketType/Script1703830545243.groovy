...
WebUI.selectOptionByLabel(findTestObject('Object Repository/Page_Watsons User/selection_TicketType'), watsonsUser_TicketType, true)
...
for (int row = 0; row < rows_count; row++) {
	...
	String columnTicketType_value = Columns_row.get(4).getText()
	
	
	if (columnTicketType_value == watsonsUser_TicketType || columnTicketType_value.contains(watsonsUser_TicketType)) {
		println("This row has: $watsonsUser_TicketType")
		isMatch = true
	...
	while (rows_count == 10 && attribute == 'go to next page' && disabled == null) {
		...
		table_row = Table.findElements(By.tagName('tr'))
		 ...
		 for (int row = 0; row < rows_count; row++) {
			 ...
			 String columnRole_value = Columns_row.get(4).getText()
