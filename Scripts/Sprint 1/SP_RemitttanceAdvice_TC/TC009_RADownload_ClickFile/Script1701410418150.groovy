...
for (int row = 0; row < rows_count; row++) {
	...
	println "$vendor_code, $vendor_name, $vendor_RAnumber"
	
	if (vendor_code == "1075955" && vendor_name == "SANXIAO PHILIPPINES INCORPORATED" && vendor_RAnumber == "74305" && vendor_Status == "Unread") {
		...
		for (row = 0; row < rows_count; row++) {
			...
			if (vendor_code == "1075955" && vendor_name == "SANXIAO PHILIPPINES INCORPORATED" && vendor_RAnumber == "74305" && vendor_Status == "Unread") {
				println "Status is $vendor_Status after file download"
				
				break;
			}
			
			else {
				println"Status not match"
			}
		}
